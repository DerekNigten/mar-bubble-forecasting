"""
preprocessing_btc.py
Loads and validates the two outputs from marx_estimation_btc_monthly.R:
...
PARAMS_PATH   = ROOT / "data" / "processed" / "btc_mar_parameters_monthly.csv"
FILTERED_PATH = ROOT / "data" / "processed" / "btc_filtered_monthly.csv"
...
def _parse_mar_date(date_str: str) -> pd.Timestamp:
    """Parse 'YYYY-MM' → Timestamp('YYYY-MM-01')."""
...
def load_filtered_series(path: Path = FILTERED_PATH) -> pd.DataFrame:
    """
    Load HP-filtered Bitcoin prices.
